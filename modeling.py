    ...
    def generate(self, input_text, 
        ...
        with torch.no_grad():
            ...
            if pondering is None:
                ponder_input_ids = None
            else:
                ponder_input_ids = self.tokenizer(input_text_keys, return_tensors="pt").input_ids.to(self.device)
                max_len = ponder_input_ids.shape[-1] + max_new_tokens
            ...
            if mode == 'baseline':
                outputs = self.model.generate(input_ids, max_length=max_len, num_return_sequences=1,
                                    ...
                                    pondering=pondering, alpha=alpha, ponder_inputs=ponder_input_ids, **kwargs)
        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                if pondering is not None:
                    ...
                    #     answer.append(self.tokenizer.decode([i]))
                    # print(answer)
                    # print('llama prob')
                    # llama_prob = outputs.softmax(-1)[prefix_ids.shape[-1] - 1: -1, :]
                    # print(llama_prob[range(continue_ids.shape[0]), continue_ids])
                    # print(torch.prod(llama_prob[range(continue_ids.shape[0]), continue_ids][4:-1]))
                    ...
                    # print('ponder prob')
                    # ponder_prob = ponder_outputs.softmax(-1)[prefix_keys_ids.shape[-1] - 1: -1, :]
                    # print(ponder_prob[range(continue_ids.shape[0]), continue_ids])
                    # print(torch.prod(ponder_prob[range(continue_ids.shape[0]), continue_ids][4:-1]))
                    # diff_prob = ponder_prob / llama_prob
                    # print('diff prob')
                    # print(diff_prob[range(continue_ids.shape[0]), continue_ids])
                    # print(torch.prod(diff_prob[range(continue_ids.shape[0]), continue_ids][4:-1]))
                    ponder_outputs = ponder_outputs.log_softmax(-1)  # logits to log probs
                    # skip tokens in the prompt -- we only care about the answer
                    ponder_logits = ponder_outputs[prefix_keys_ids.shape[-1] - 1: -1, :]
                    # get logprobs for each token in the answer
                    # log_probs = ponder_logits[range(ponder_logits.shape[0]), continue_ids].sum().item()

                    final_logits = ponder_logits - logits
                    final_logits = ponder_logits + alpha * final_logits
                    # if post_softmax:
                    # final_logits = diff_logits.log_softmax(dim=-1)
                    if alpha == 0:
                        final_logits = ponder_logits
                    else:
                ...
                if pondering is not None:
                    ...
                    input_keys_ids = self.tokenizer(input_text_keys, return_tensors="pt").input_ids.to(self.device)
                    prefix_keys_ids = self.tokenizer(input_text1_keys, return_tensors="pt").input_ids.to(self.device)
                    ponder_outputs = self.model(input_keys_ids)[0]
                    shift_len = prefix_keys_ids.shape[1] - prefix_ids.shape[1]
                ...
                for seq_i in range(prefix_ids.shape[-1] - 1, input_ids.shape[-1] - 1):
                    ...
                    if pondering is not None:
                        softmax_mature_layer = F.softmax(ponder_outputs[:, shift_len + seq_i, :], dim=-1)
                    else:
                    ...
                    if pondering is not None:
                        log_softmax_mature_layer = F.log_softmax(ponder_outputs[:, shift_len + seq_i, :], dim=-1)
                    else:
                ...
                # pdb.set_trace()
                if pondering is not None:
                    final_logits = ponder_outputs[0, prefix_keys_ids.shape[-1] - 1:-1]
                else:
                    final_logits = dict_outputs[mature_layer][0, prefix_ids.shape[-1] - 1:-1]
                final_logits = final_logits.log_softmax(dim=-1)
                base_logits = base_logits.log_softmax(dim=-1)
