...
class GenerationMixin:
    ...
    def generate(
        ...
        pondering: Optional[str] = None,
        alpha: Optional[float] = 10,
        ponder_inputs: Optional[torch.Tensor] = None,
        **kwargs,
        ...
        if pondering is not None:
            if self.config.is_encoder_decoder:
                logger.warning(
                    "Pondering not implemented for encoder-decoder architecture yet."
                )
            
            ponder_input_ids = ponder_inputs
            ponder_attention_mask = self._prepare_attention_mask_for_generation(
                ponder_input_ids, generation_config.pad_token_id, generation_config.eos_token_id
            )
        ...
        input_ids_seq_length = input_ids.shape[-1] if pondering is None else ponder_input_ids.shape[-1]
        ...
        if is_greedy_gen_mode and pondering is not None:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    f"num_return_sequences has to be 1, but is {generation_config.num_return_sequences} when doing"
                    " greedy search."
            ...
            # 11. run greedy search
            return self.pondering_contrastive_greedy_decode(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                ...
                streamer=streamer,
                alpha=alpha,
                ponder_input_ids=ponder_input_ids,
                ponder_attention_mask=ponder_attention_mask,
                **model_kwargs,
            )
    ...
    def pondering_contrastive_greedy_decode(
        self,
        input_ids: torch.LongTensor,
        ponder_input_ids: torch.LongTensor,
        ponder_attention_mask: torch.LongTensor,
        relative_top: float = 0.,
        alpha: float = 10,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        ponder_model_kwargs = copy.deepcopy(model_kwargs)
        ponder_model_kwargs["attention_mask"] = ponder_attention_mask
        this_peer_finished = False  # used by synced_gpus only
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            ponder_model_inputs = self.prepare_inputs_for_generation(ponder_input_ids, **ponder_model_kwargs)
            ...
            base_logits = outputs[0][:, -1, :]
            final_logits = ponder_outputs[0][:, -1, :]
            # import pdb
            # pdb.set_trace()
            # if relative_top > 0.0:
            #     final_logits = self.relative_top_filter(final_logits, relative_top)
            ...
            logits = final_logits + alpha * (final_logits - base_logits)
            next_token_logits = logits
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            ponder_input_ids = torch.cat([ponder_input_ids, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
            ponder_model_kwargs = self._update_model_kwargs_for_generation(
                ponder_outputs, ponder_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
